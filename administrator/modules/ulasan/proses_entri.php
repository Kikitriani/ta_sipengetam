...
// jika user sudah login, maka jalankan perintah untuk insert
  ...
  if (isset($_POST['simpan'])) {
    ...
    $result   = mysqli_query($mysqli, "SELECT max(id_riwayat) as kode FROM tbl_riwayat")
    ...
    if(in_array($extension, $allowed_extensions)) {
      if($ukuran_file <= 5000000) {
        if(move_uploaded_file($tmp_file, $path)) {
          ...
          $insert = mysqli_query($mysqli, "INSERT INTO tbl_riwayat(id_riwayat, id_tempat, gambar, tanggal, status, keterangan, created_user) VALUES('$id', '$id_tempat', '$nama_file_enkripsi', '$tgl_riwayat', '$status', '$keterangan', '$created_user')")or die('Ada kesalahan pada query insert : ' . mysqli_error($mysqli));
          if ($insert) {
            header('location: ../../main.php?module=riwayat&pesan=1');
          }
        }
      }
    }
