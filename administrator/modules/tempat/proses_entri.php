...
// jika user sudah login, maka jalankan perintah untuk insert
  ...
  if (isset($_POST['simpan'])) {
    ...
    $result   = mysqli_query($mysqli, "SELECT max(id_tempat) as kode FROM tbl_tempat")
                or die('Ada kesalahan pada query tampil data id: '. mysqli_error($mysqli));
    
    $data     = mysqli_fetch_assoc($result);
    $id       = $data['kode'] + 1;

    $nama_tempat  = mysqli_real_escape_string($mysqli, trim($_POST['nama_tempat']));
    ...
    if(in_array($extension, $allowed_extensions)) {
      if($ukuran_file <= 5000000) {
        if(move_uploaded_file($tmp_file, $path)) {
          ...
          $insert = mysqli_query($mysqli, "INSERT INTO tbl_tempat(id_tempat, nama_tempat, gambar, lat, lng, lokasi, keterangan, created_user) VALUES('$id', '$nama_tempat', '$nama_file_enkripsi', '$lat', '$lng', '$lokasi', '$keterangan', '$created_user')")or die('Ada kesalahan pada query insert : ' . mysqli_error($mysqli));
          if ($insert) {
            header('location: ../../main.php?module=tempat&pesan=1');
          }
        }
      }
    }
